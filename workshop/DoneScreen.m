...
- (void)viewWillAppear:(BOOL)animated{
    NSDate *proTask = [[NSUserDefaults standardUserDefaults] objectForKey:@"Done"];
    ...
    if(proArray){
        ...
        doneTasksArray=[NSMutableArray array];
    }
    [self.doneTableView reloadData];
}
- (void)saveEditAction:(TaskData *)editedTask {
    NSInteger index = [doneTasksArray indexOfObject:editedTask];;
    if (index != NSNotFound) {
        if([editedTask.state isEqual:@"DONE"]){
            doneTasksArray[index] = editedTask;
            ...
            [doneTasksArray removeObjectAtIndex:index];
        }
        
        NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
        NSData *data = [NSKeyedArchiver archivedDataWithRootObject:doneTasksArray];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"donecell"];
        cell = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:@"donecell"];
    if(!_isfilter){
        NSDate *data = [[NSUserDefaults standardUserDefaults] objectForKey:@"Done"];
...
- (NSArray<UITableViewRowAction *> *)tableView:(UITableView *)tableView editActionsForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    UITableViewRowAction *deleteAction = [UITableViewRowAction rowActionWithStyle:UITableViewRowActionStyleDestructive title:@"Delete" handler:^(UITableViewRowAction * _Nonnull action, NSIndexPath * _Nonnull indexPath) {
        ...
        UIAlertAction * save = [UIAlertAction actionWithTitle:@"Delete" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
            if(!_isfilter){
                [doneTasksArray removeObjectAtIndex:indexPath.row];
            }else{
                switch (indexPath.section) {
                    case 0:
            ...
            NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
            NSData *Mdata = [NSKeyedArchiver archivedDataWithRootObject:doneTasksArray];
    ...
    UITableViewRowAction *editAction = [UITableViewRowAction rowActionWithStyle:UITableViewRowActionStyleNormal title:@"Edit" handler:^(UITableViewRowAction * _Nonnull action, NSIndexPath * _Nonnull indexPath) {
        ...
        TaskData * selected ;
        if(!_isfilter){
            selected =doneTasksArray[indexPath.row];
        }else{
            switch (indexPath.section) {
                case 0:
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    if(!_isfilter){
        headerLabel.text = @"All Done Tasks";
    }else{
        switch (section) {
            case 0:
